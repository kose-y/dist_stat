...
if __name__=='__main__':
    ...
    parser.add_argument('--nrow', dest='nrow', action='store', default=10000)
    parser.add_argument('--ncol', dest='ncol', action='store', default=10000)
    parser.add_argument('--iter', dest='iter', action='store', default=10000)
    parser.add_argument('--lambda', dest='lmbda', action='store', default=0.00001)
    ...
    n = int(args.ncol); p = int(args.nrow)
    X = torch.randn((p, n))
    ...
    cox_driver.run(int(args.iter), tol=1e-8,check_interval=100, check_obj=True)
