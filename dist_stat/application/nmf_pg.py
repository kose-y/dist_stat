...
class NMF():
    ...
    def update_V(self, eps=1e-9):
        Wt = self.W.t()
        XWt =  distmat.mm(self.data, Wt)
        WWt =  distmat.mm(self.W, Wt)
        VWWt = distmat.mm(self.V, WWt)
        sigma_k = 1.0/(2*((WWt**2).sum()).sqrt()+eps)
        self.V = (self.V - (VWWt - XWt)*sigma_k).apply(torch.clamp, min=0)
    def update_W(self, eps=1e-9):
        VtX  = distmat.mm(self.V.t(), self.data, out_sizes=self.W.sizes) 
        VtV  = distmat.mm(self.V.t(), self.V)
        VtVW = distmat.mm(VtV, self.W)
        tau_k = 1.0/(2*((VtV**2).sum()).sqrt()+eps)
        self.W = (self.W - (VtVW-VtX)*tau_k).apply(torch.clamp, min=0)
